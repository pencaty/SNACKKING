...
if( (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['submit'])) || $android )
{
    ...
    $bland=(int)($_POST['bland']);
    $nutty=(int)($_POST['nutty']);
    $salty=(int)($_POST['salty']);
    $greasy=(int)($_POST['greasy']);
    $crispy=(int)($_POST['crispy']);
    $moist=(int)($_POST['moist']);

    if(empty($sweet) && empty($spicy) && empty($bland) && empty($nutty) && empty($salty) && empty($greasy) && empty($crispy) && empty($moist)) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        while($row=$stmt->fetch(PDO::FETCH_ASSOC)) { // $row[] : 기존 각 keyword 별 점수
            if($row['Name'] == $name){
                ...
                $bland = $bland + $row['Bland'];
                $nutty = $nutty + $row['Nutty'];
                ...
                switch($first_index) {
                    ...
                    break;
                    case 2 : $keyword_one = 'Bland';
                    break;
                    case 3 : $keyword_one = 'Nutty';
                    break;
                ...
                switch($second_index) {
                    ...
                    case 1 : $keyword_two = 'Spicy';
                        break;
                    case 2 : $keyword_two = 'Bland';
                        break;
                    case 3 : $keyword_two = 'Nutty';
                        break;
                ...
                switch($third_index) {
                    ...
                    case 1 : $keyword_three = 'Spicy';
                        break;
                    case 2 : $keyword_three = 'Bland';
                        break;
                    case 3 : $keyword_three = 'Nutty';
                        break;
...
/*                $update_score = $con->prepare('UPDATE Snack_Score SET Sweet = :sweet, Spicy = :spicy, Bland = :bland, Nutty = :nutty WHERE Name = :name');
                $update_score->bindParam(':name', $name);
                $update_score->bindParam(':sweet', $sweet);
                $update_score->bindParam(':spicy', $spicy);
                $update_score->bindParam(':bland', $bland);
                $update_score->bindParam(':nutty', $nutty);
                $update_score->execute();
                break;*/

                $update_score = $con->prepare('UPDATE Snack_Score SET Keyword_One = :keyword1, Keyword_Two = :keyword2, Keyword_Three = :keyword3, Sweet = :sweet, Spicy = :spicy, Bland = :bland, Nutty = :nutty, Salty = :salty, Greasy = :greasy, Crispy = :crispy, Moisturized = :moist WHERE Name = :name');
                $update_score->bindParam(':name', $name);
...
if( !$android )
{
    ...
    <form action="<?php $_PHP_SELF ?>" method="POST">
        ...
        Bland: <input type = "text" name = "bland" /><br>
        Nutty: <input type = "text" name = "nutty" />
